...
function KVStore() {
  ...
  usePageHeader('Key-Value Store', 'Manage application data and configuration');
  const [kvPairs, setKvPairs] = createSignal([]);
  const [filteredPairs, setFilteredPairs] = createSignal([]);
  const [isLoading, setIsLoading] = createSignal(true);
  const [searchQuery, setSearchQuery] = createSignal('');
  const [showAddModal, setShowAddModal] = createSignal(false);
  const [editingPair, setEditingPair] = createSignal(null);
  const [newKey, setNewKey] = createSignal('');
  const [newValue, setNewValue] = createSignal('');
  const [newTtl, setNewTtl] = createSignal(24);
  const [showValues, setShowValues] = createSignal({});
  
  const kvStore = useKVStore();
  const { info, error } = useNotifications();

  // Load common KV pairs
  const loadKVPairs = async () => {
    setIsLoading(true);
    try {
      // Common keys to check - in production you'd have a list endpoint
      const commonKeys = [
      ...
      const pairs = await kvStore.getMultiple(commonKeys);
      setKvPairs(pairs);
  ...
  return (
    <div class="max-w-7xl mx-auto p-6">
      {/* Header */}
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-2">KV Store Management</h1>
        <p class="text-slate-600 dark:text-slate-400">
          Manage key-value pairs stored in the MCP server. View, edit, and organize your application data.
        </p>
      </div>
      {/* Stats */}
      ...
      {/* Search Results Info */}
      <Show when={searchQuery()}>
        <div class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800/50 rounded-lg">
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead class="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                <tr>
                  <th class="text-left p-4 font-medium text-slate-900 dark:text-white">Key</th>
                  <th class="text-left p-4 font-medium text-slate-900 dark:text-white">Value</th>
                  <th class="text-left p-4 font-medium text-slate-900 dark:text-white">Type</th>
                  <th class="text-left p-4 font-medium text-slate-900 dark:text-white">Size</th>
                    ...
                    <tr class="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700">
                      <td class="p-4">
                        <div class="flex items-center gap-2">
                          ...
                          <code class="text-sm font-mono bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white px-2 py-1 rounded">
                            {pair.key}
                          </code>
                        </div>
                      </td>
                      <td class="p-4 max-w-xs">
                        <div class="flex items-center gap-2">
                          <Show when={showValues()[pair.key]} fallback={
                            ...
                            <pre class="text-xs bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white p-2 rounded border border-slate-200 dark:border-slate-600 max-h-20 overflow-y-auto whitespace-pre-wrap">
