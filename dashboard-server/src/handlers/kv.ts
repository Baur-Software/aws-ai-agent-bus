...
export class KVHandler {
  ...
  static async set({ key, value, ttl_hours = 24 }) {
    ...
    const result = await DynamoDBService.putItem({
      key: `KV#${key}`,
      value,
      expires_at: ttl,
      type: 'kv',
    ...
    // Emit KV change event for real-time updates
    try {
      await EventsHandler.send({
        detailType: 'KV Store Updated',
        detail: {
          operation: 'set',
          key,
      ...
      console.warn('Failed to emit KV event:', error);
